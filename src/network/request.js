...
export function get(url, config) {
  ...
  instance.interceptors.response.use(
    ...
    (err) => {
      ...
      // } else {
      //   //ElMessage.error("特丽丽被玩坏了QAQ");(¯﹃¯)get");
      //   store.state.isLoading = false;
      // }
    },
  );
...
export function post(url, data, headers) {
  ...
  instance.interceptors.response.use(
    (config) => {
      const code = config.data.code;
      if (code && code !== 200)
        console.error(config.data.message || '未知错误, 请打开控制台查看');
      return config;
    },
    (err) => {
      console.log(err);
      ...
      // } else {
      //   //ElMessage.error("特丽丽被玩坏了QAQ");(¯﹃¯)post");
      //   store.state.isLoading = false;
      // }
    },
  );
